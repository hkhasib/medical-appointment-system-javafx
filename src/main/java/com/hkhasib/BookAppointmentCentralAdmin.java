...
public class BookAppointmentCentralAdmin implements Initializable {
    ...
    private String dbhost, dbuser, dbname, dbport, dbpass, dayName, doctorUsername, patientUsername, day, firstName, lastName, phone, specialities;
    private LocalTime from, to;
    private String note = "N/A", emergency = "No";
    private String readRoutine = "Select * from routines";
    private int doctorID, patientID, userID, docID, patID;
    ...
    @FXML
    private CheckBox emergencyChecker;
    @FXML
    private ComboBox<String> doctorSelector;
    @FXML
    private ComboBox<String> patientSelector;
    @FXML
    private TextField nameField, specialityField, phoneField, statusText;
    @FXML
    private TextArea noteField;
    @FXML
    private TableView<Routine> routineTable;
    @FXML
    private TableColumn<Routine, String> dayCol;
    @FXML
    private TableColumn<Routine, String> fromCol;
    @FXML
    private TableColumn<Routine, String> toCol;
    @FXML
    private TableColumn<Routine, String> idCol;


    private LocalDate date;


    public BookAppointmentCentralAdmin() {
    ...
    private void getPatientDetails() {
        patientUsername = patientSelector.getValue();
        try {
            Connection connection = DriverManager.getConnection("jdbc:mysql://" + dbhost + ":" + dbport + "/" + dbname + "?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC", dbuser, dbpass);
            Statement statement = connection.createStatement();
            ResultSet rs = statement.executeQuery("Select * from patients where username='" + doctorUsername + "'");
            while (rs.next()) {
                firstName = rs.getString(2);
                lastName = rs.getString(3);
                phone = rs.getString(6);
    ...
    @FXML
    private void timeSlotMaker() {

    }

    @FXML private void returnToDashboard() throws IOException {
        App.setRoot("FrontDeskDashboard");
    }

    @FXML
    private void checkRoutine() throws IOException {
        fetchDBData();
    ...
    private void getPatientID() throws IOException {
        fetchDBData();
        String readdata = "Select * from patients where username='" + patientSelector.getValue() + "'";
        try {
            Connection connection = DriverManager.getConnection("jdbc:mysql://" + dbhost + ":" + dbport + "/" + dbname + "?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC", dbuser, dbpass);
            Statement statement = connection.createStatement();
    ...
    @FXML
    private void tableViewClickAction() {
    ...
    @FXML
    private void postAppointment() throws IOException {
        ...
        String postdata = "insert into Appointments values(" + patientID + "," + doctorID + ",'" + date + "','" + from + "','" + note + "','" + emergency + "')";
        try {
            Connection connection = DriverManager.getConnection("jdbc:mysql://" + dbhost + ":" + dbport + "/" + dbname + "?useUnicode=true&useJDBCCompliantTimezoneShift=true&useLegacyDatetimeCode=false&serverTimezone=UTC", dbuser, dbpass);
            Statement statement = connection.createStatement();
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        try {
            ...
            ResultSet rd = statement.executeQuery("SELECT * from patients");
            while (rd.next()){
                patientNames.add(rd.getString("username"));
            }
            connection.close();
        } catch (SQLException throwables) {
            throwables.printStackTrace();
        ...
        patientSelector.setItems(patientNames);
